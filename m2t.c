...
MoneySplit money_splt_in_two_halfs(char *money) // recebe o valor "5432178,99"
{
    ...
    if (separator != NULL)
    {
        ...
        strncpy(money_split.l_half, money, char_amount); // copia para dentro de "l_half" o valor "5432178" que vem antes da virgula
    ...
    {
        strcpy(money_split.l_half, money); // Se não tiver separador copia o valor de "money" para dentro de "l_half"
...
SplitInGroupsOfThree split_in_groups_of_three(char *data)
{
    ...
    while (sigot.i > 0)
    {
        ...
        strncpy(sigot.groups[sigot.group_index], &data[start], size); // Copia o trecho do grupo
